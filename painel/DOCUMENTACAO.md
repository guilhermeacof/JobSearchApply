...
| `GET /api/cvs` | Lista os currículos gerados por vaga (pastas em `documents/applications/` com `cv_draft.tex`): `{cvs:[{id,label,temPdf}]}`. |
| `GET /api/cv?id=…` | Conteúdo do `cv_draft.tex` (LaTeX) para edição. |
| `POST /api/cv` | Salva `{id, content}` no `cv_draft.tex` e **recompila** com `lualatex` (2 passadas) → `{ok, compilado, log}`. |
| `GET /api/cv/pdf?id=…` | Serve o PDF do currículo (inline). Se ainda não houver PDF, compila antes. Prioriza o `cv_draft.pdf` recompilado. |
| `GET /api/cv/download?id=…&fmt=pdf\|tex` | Baixa o PDF ou o `.tex` com nome amigável (cabeçalho ASCII + `filename*` UTF-8). |
...
11. **Seleção em massa de vagas** — cada vaga tem um **checkbox** (e há "selecionar todas
    ...
    - **"Preparar candidaturas"** — roda o `/apply` para cada vaga selecionada, uma a uma,
      transmitindo o progresso no console (com aviso se alguma já foi enviada).
12. **Meu currículo (por vaga)** — lista os currículos que o sistema gerou para cada
    candidatura. Para o selecionado: **Ver PDF** (abre em nova aba), **Baixar PDF**,
    **Editar texto** (abre o LaTeX numa caixa, com **Salvar e recompilar** via `lualatex` e
